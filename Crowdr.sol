...
contract Crowdr is AutomationCompatibleInterface {
    ...
    function claimRefund(uint256 _index) validIndex(_index) external {
        require(campaigns[_index].duration + campaigns[_index].endTime > block.timestamp, "Project Funding Time Not Expired");
        require(campaigns[_index].refundPolicy == RefundPolicy.REFUNDABLE 
        && campaigns[_index].amountRaised > campaigns[_index].fundingGoal, "Funding goal not reached");
        ...
        require(!campaigns[_index].refundClaimed[contributorIndex], "Already claimed refund amount");
        
        campaigns[_index].refundClaimed[contributorIndex] = true;
        ...
        returns (bool upkeepNeeded, bytes memory  /* performData */)
    {
          for(uint256 i = 0; i < campaigns.length; i++){
            ...
            if(campaigns[i].duration + campaigns[i].endTime > block.timestamp && 
            campaigns[i].amountRaised < campaigns[i].fundingGoal ){
                upkeepNeeded = true
            }
        }
